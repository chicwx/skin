...
- (id)bf_addDeallocBlock:(void (^)())deallocBlock {
    if (deallocBlock == nil) {
        return nil;
    }
    ...
    for (BFDeallocBlockExecutor *executor in deallocBlocks) {
        if (executor.deallocBlock == deallocBlock) {
            return nil;
        }
    }
    
    BFDeallocBlockExecutor *executor = [BFDeallocBlockExecutor executorWithDeallocBlock:deallocBlock];
